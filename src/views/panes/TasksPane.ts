...
export function renderTasksPane(parent: HTMLElement, ctx: ViewContext): void {
...

  {
    ...
    if (isSmart) {
      ...
      makeEditableName(nameEl, {
        ...
        display: ctx.settings.prettyTitles
          ? prettifyName(list.name)
          : list.name,
        alwaysEditable: true,
  ...
  if (!isSmart && list) {
    ...
    sortBtn.setAttribute(
      "aria-label",
      `Sort: ${current?.label ?? "Custom order"}`,
    );
    sortBtn.addEventListener("click", (e) => {
      ...
      if (hasGroups)
        menu.addItem((i) => i.setTitle("Sort tasks").setIsLabel(true));
    ...
    // A control that cannot do anything is worse than no control, so the
    // layout toggle is absent rather than inert while the wall is off.
    if (ctx.settings.postItView) {
          ...
          .onClick(() => {
            void import("../../ui/PromptModal").then(({ PromptModal }) => {
              new PromptModal(ctx.app, {
                ...
                cta: "Add",
                onSubmit: (name) =>
                  void ctx.mutator.createSection(list.path, name),
              }).open();
            });
          }),
      );
...
function renderTasks(
    ...
    for (const run of runs) {
      ...
      run.rows.forEach((row, index) => {
        ...
        makeDragSortable(row, {
          ...
          containers,
          onDrop: (from, to) =>
            void ctx.mutator.reorder(run.tasks[from], run.tasks, to),
  ...
  if (opts.starredFirst) {
    ...
    if (starred.length) {
      ...
      runs.push({
        ...
        rows: starred.map((t) =>
          postit
            ? renderTaskCard(el, t, ctx, {
                showList: opts.showList,
                showSection: true,
              })
            : renderTaskRow(el, t, ctx, {
                showList: opts.showList,
                showSection: true,
              }),
        ),
  ...
  const firstHeading = Math.min(
    ...opts.sections.map((sec) => sec.line),
    Infinity,
  );
  ...
  if (opts.groupSort === "custom" && heads.length > 1) {
    heads.forEach((head, index) => {
      ...
      makeDragSortable(head, {
        ...
        siblings: () => heads,
        onDrop: (from, to) =>
          void ctx.mutator.moveSection(path, bounds[from].line, to),
      });
    });
  }
